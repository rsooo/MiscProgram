...
function inviteNearUser(){
	...
	mysql_query("SET NAMES utf8");
	//周囲の招待可能な人を絞るクエリ
	$query = sprintf("SELECT id, username, latitude, longitude FROM loginuserlist WHERE abs(latitude - %d) < 100000 
    AND abs(longitude - %d) < 100000
    AND status = 'WAIT'
    AND acceptinvite = 1
	AND NOT id = %d", $my_latitude, $my_longitude, $userId);
	putlog($query);
...
function enterChatroom(){
	...
	$chatroomNo = getNewChatroomNo($db);
	if(chatroomNo == -1){
...
//	$query2 = sprintf("INSERT INTO chatroom values(%s,'%s','0:20',%s, %s, '%s')", $userid, $mes, $chatroomNo, $sequence, $username);
	...
	$date = getdatetime();
	$exit_mes = mb_convert_encoding($username . "が退室しました。", 'utf8', 'sjis-win');	
...
function deleteLoginData(){
	...
	$db = mysql_connect("mysql120.db.sakura.ne.jp", "stress-free", "greedgreed");
	mysql_select_db("stress-free", $db);
	$query = sprintf("DELETE FROM loginuserlist WHERE username = '%s';", 
